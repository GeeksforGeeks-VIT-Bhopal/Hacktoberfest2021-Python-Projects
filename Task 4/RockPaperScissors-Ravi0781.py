...
style1 = Style()
style1.configure('W.TButton', foreground = 'green', font = ('calibri', 20, 'bold'))
...
def clicked2():
    global cho
    lb9.configure(text = 'You picked Rock')
    cho = 0
def clicked3():
    global cho
    lb9.configure(text='You picked Paper')
    cho = 1
def clicked4():
    global cho
    lb9.configure(text='You picked Scissor')
    cho = 2
...
def clicked1():
    global cho
    if cho == 3:
        lb1.configure(text='Please choose an option to play!', foreground = 'violet')
        return
